...
function buildResult(rel, mongo, jsonb, stats, flatRows) {
  ...
  return {
    ...
    row_counts: {
      insert: { pg_relational: rel.rowCounts.insert, mongodb: mongo.rowCounts.insert, pg_jsonb: jsonb.rowCounts.insert },
      selectAll: { pg_relational: rel.rowCounts.selectAll, mongodb: mongo.rowCounts.selectAll, pg_jsonb: jsonb.rowCounts.selectAll },
      selectFilter: { pg_relational: rel.rowCounts.selectFilter, mongodb: mongo.rowCounts.selectFilter, pg_jsonb: jsonb.rowCounts.selectFilter },
      selectIndexed: { pg_relational: rel.rowCounts.selectIndexed, mongodb: mongo.rowCounts.selectIndexed, pg_jsonb: jsonb.rowCounts.selectIndexed },
...
async function runBenchmark(onProgress) {
  ...
  emit({ step: 'flatten', message: `Building flat rows (${revisions.length} revisions)...` });
  ...
  try {
    emit({ step: 'pg_rel', message: '[1/3] PG Relational — DROP + CREATE tables...' });
    const relResult = await benchPgRelationalWithProgress(pgClient, categories, pages, revisions, emit);

    emit({ step: 'mongo', message: '[2/3] MongoDB — DROP collection...' });
    const mongoResult = await benchMongoWithProgress(mongoDB, flatRows, emit);

    emit({ step: 'pg_jsonb', message: '[3/3] PG JSONB — DROP + CREATE table...' });
    const jsonbResult = await benchPgJsonbWithProgress(pgClient, flatRows, emit);
...
async function benchPgRelationalWithProgress(pg, categories, pages, rawRevisions, emit) {
  const seen = new Set();
  const revisions = rawRevisions.filter(r => { if (seen.has(r.rootid)) return false; seen.add(r.rootid); return true; });
  const R = {};
  ...
  await pg.query('DROP TABLE IF EXISTS bench_page');
  await pg.query('DROP TABLE IF EXISTS bench_category');
  await pg.query(`CREATE TABLE bench_category (id SERIAL PRIMARY KEY, rootid UUID UNIQUE DEFAULT gen_random_uuid(), prev_id INTEGER, name VARCHAR(100), date INTEGER, time INTEGER, date_time BIGINT)`);
  await pg.query(`CREATE TABLE bench_page (id SERIAL PRIMARY KEY, rootid INTEGER UNIQUE NOT NULL, prev_id INTEGER, category_id INTEGER REFERENCES bench_category(id), page_title VARCHAR(500), date INTEGER, time INTEGER, date_time BIGINT)`);
  await pg.query(`CREATE TABLE bench_revision (id SERIAL PRIMARY KEY, rootid INTEGER UNIQUE NOT NULL, prev_id INTEGER, page_id INTEGER REFERENCES bench_page(id), username VARCHAR(255), timestamp VARCHAR(30), comment TEXT, content TEXT, date INTEGER, time INTEGER, date_time BIGINT)`);

  emit({ step: 'pg_rel_insert', message: '[1/3] PG Relational — INSERT 3 tables...' });
  const { time: tInsert } = await timer(async () => {
    const catVals = [];
    const catPhs = categories.map((c, i) => { const off = i * 4; catVals.push(c.name, c.date, c.time, c.date_time); return `($${off+1},$${off+2},$${off+3},$${off+4})`; });
    await pg.query(`INSERT INTO bench_category (name, date, time, date_time) VALUES ${catPhs.join(',')}`, catVals);
    const catRows = await pg.query('SELECT id, name FROM bench_category');
    const catIdMap = new Map(); catRows.rows.forEach(r => catIdMap.set(r.name, r.id));
    const maxPageBatch = Math.floor(65535 / 6);
    for (let i = 0; i < pages.length; i += maxPageBatch) {
      const batch = pages.slice(i, Math.min(i + maxPageBatch, pages.length));
      const vals = []; const phs = batch.map((p, bi) => { const off = bi * 6; vals.push(p.rootid, catIdMap.get(p.category), p.page_title, p.date, p.time, p.date_time); return `($${off+1},$${off+2},$${off+3},$${off+4},$${off+5},$${off+6})`; });
      await pg.query(`INSERT INTO bench_page (rootid, category_id, page_title, date, time, date_time) VALUES ${phs.join(',')}`, vals);
    }
    const pageRows = await pg.query('SELECT id, rootid FROM bench_page');
    const pageIdMap = new Map(); pageRows.rows.forEach(r => pageIdMap.set(r.rootid, r.id));
    const maxRevBatch = Math.floor(65535 / 9);
    for (let i = 0; i < revisions.length; i += maxRevBatch) {
      const batch = revisions.slice(i, Math.min(i + maxRevBatch, revisions.length));
      const vals = []; const phs = batch.map((rev, bi) => { const off = bi * 9; vals.push(rev.rootid, rev.prev_id, pageIdMap.get(rev.pageid), rev.username, rev.timestamp, rev.comment, rev.content, rev.date, rev.date_time); return `($${off+1},$${off+2},$${off+3},$${off+4},$${off+5},$${off+6},$${off+7},$${off+8},$${off+9})`; });
      await pg.query(`INSERT INTO bench_revision (rootid, prev_id, page_id, username, timestamp, comment, content, date, date_time) VALUES ${phs.join(',')}`, vals);
    }
  });
  R.insert = tInsert;

  emit({ step: 'pg_rel_select', message: '[1/3] PG Relational — SELECT * (JOIN 3 tables)...' });
  const { time: tAll, result: rAll } = await timer(() => pg.query(`SELECT r.*, p.page_title, p.rootid AS page_rootid, c.name AS category FROM bench_revision r JOIN bench_page p ON r.page_id = p.id JOIN bench_category c ON p.category_id = c.id`));
  R.selectAll = tAll;

  const filterCat = 'computer_science_research';
  emit({ step: 'pg_rel_filter', message: '[1/3] PG Relational — SELECT filter (no index)...' });
  const { time: tFilter, result: rFilter } = await timer(() => pg.query(`SELECT r.*, p.page_title, c.name AS category FROM bench_revision r JOIN bench_page p ON r.page_id = p.id JOIN bench_category c ON p.category_id = c.id WHERE c.name = $1`, [filterCat]));
  R.selectFilter = tFilter;

  emit({ step: 'pg_rel_index', message: '[1/3] PG Relational — CREATE INDEX (B-Tree)...' });
  const { time: tIdx } = await timer(() => pg.query('CREATE INDEX idx_bench_page_cat ON bench_page(category_id)'));
  R.createIndex = tIdx;

  emit({ step: 'pg_rel_indexed', message: '[1/3] PG Relational — SELECT filter (with index)...' });
  const { time: tIdxQ, result: rIdxQ } = await timer(() => pg.query(`SELECT r.*, p.page_title, c.name AS category FROM bench_revision r JOIN bench_page p ON r.page_id = p.id JOIN bench_category c ON p.category_id = c.id WHERE c.name = $1`, [filterCat]));
  R.selectIndexed = tIdxQ;

  emit({ step: 'pg_rel_update', message: '[1/3] PG Relational — UPDATE 1 row...' });
  const { time: tUpd } = await timer(() => pg.query('UPDATE bench_revision SET comment = $1 WHERE id = 1', ['updated_comment']));
  R.update = tUpd;

  emit({ step: 'pg_rel_delete', message: '[1/3] PG Relational — DELETE 1 row...' });
  const { time: tDel } = await timer(() => pg.query('DELETE FROM bench_revision WHERE id = 1'));
  R.delete = tDel;

  emit({ step: 'pg_rel_storage', message: '[1/3] PG Relational — Measuring storage...' });
  const sz = await pg.query(`SELECT (pg_table_size('bench_category') + pg_table_size('bench_page') + pg_table_size('bench_revision')) AS data, (pg_indexes_size('bench_category') + pg_indexes_size('bench_page') + pg_indexes_size('bench_revision')) AS idx, (pg_total_relation_size('bench_category') + pg_total_relation_size('bench_page') + pg_total_relation_size('bench_revision')) AS total`);
  R.storage = { data: parseInt(sz.rows[0].data), index: parseInt(sz.rows[0].idx), total: parseInt(sz.rows[0].total) };
  R.rowCounts = {
...
async function benchMongoWithProgress(db, flatRows, emit) {
  const R = {};
  const col = db.collection('bench_mongo');
  await col.drop().catch(() => {});

  emit({ step: 'mongo_insert', message: '[2/3] MongoDB — INSERT ' + flatRows.length + ' docs...' });
  const { time: tInsert } = await timer(async () => { const docs = flatRows.map((row, i) => ({ _seq: i + 1, ...row })); await col.insertMany(docs, { ordered: false }); });
  R.insert = tInsert;

  emit({ step: 'mongo_select', message: '[2/3] MongoDB — SELECT * (find all)...' });
  const { time: tAll, result: rAll } = await timer(() => col.find({}).toArray());
  R.selectAll = tAll;

  const filterCat = 'computer_science_research';
  emit({ step: 'mongo_filter', message: '[2/3] MongoDB — SELECT filter (no index)...' });
  const { time: tFilter, result: rFilter } = await timer(() => col.find({ category: filterCat }).toArray());
  R.selectFilter = tFilter;

  emit({ step: 'mongo_index', message: '[2/3] MongoDB — CREATE INDEX (B-Tree)...' });
  const { time: tIdx } = await timer(() => col.createIndex({ category: 1 }));
  R.createIndex = tIdx;

  emit({ step: 'mongo_indexed', message: '[2/3] MongoDB — SELECT filter (with index)...' });
  const { time: tIdxQ, result: rIdxQ } = await timer(() => col.find({ category: filterCat }).toArray());
  R.selectIndexed = tIdxQ;

  emit({ step: 'mongo_update', message: '[2/3] MongoDB — UPDATE 1 doc...' });
  const { time: tUpd } = await timer(() => col.updateOne({ _seq: 1 }, { $set: { comment: 'updated_comment' } }));
  R.update = tUpd;
  ...
  emit({ step: 'mongo_storage', message: '[2/3] MongoDB — Measuring storage...' });
  const stats = await db.command({ collStats: 'bench_mongo' });
  R.storage = { data: stats.size || 0, index: stats.totalIndexSize || 0, total: (stats.storageSize || 0) + (stats.totalIndexSize || 0) };
  R.rowCounts = {
...
async function benchPgJsonbWithProgress(pg, flatRows, emit) {
  const R = {};
  await pg.query('DROP TABLE IF EXISTS bench_jsonb');
  await pg.query(`CREATE TABLE bench_jsonb (id SERIAL PRIMARY KEY, data JSONB NOT NULL DEFAULT '{}')`);

  emit({ step: 'jsonb_insert', message: '[3/3] PG JSONB — INSERT ' + flatRows.length + ' rows...' });
  const maxBatch = 65535;
  const { time: tInsert } = await timer(async () => { for (let i = 0; i < flatRows.length; i += maxBatch) { const batch = flatRows.slice(i, Math.min(i + maxBatch, flatRows.length)); const vals = []; const phs = batch.map((row, bi) => { vals.push(JSON.stringify(row)); return `($${bi+1}::jsonb)`; }); await pg.query(`INSERT INTO bench_jsonb (data) VALUES ${phs.join(',')}`, vals); } });
  R.insert = tInsert;

  emit({ step: 'jsonb_select', message: '[3/3] PG JSONB — SELECT *...' });
  const { time: tAll, result: rAll } = await timer(() => pg.query('SELECT * FROM bench_jsonb'));
  R.selectAll = tAll;

  const filterCat = 'computer_science_research';
  emit({ step: 'jsonb_filter', message: '[3/3] PG JSONB — SELECT filter (no index)...' });
  const { time: tFilter, result: rFilter } = await timer(() => pg.query(`SELECT * FROM bench_jsonb WHERE data->>'category' = $1`, [filterCat]));
  R.selectFilter = tFilter;

  emit({ step: 'jsonb_btree', message: '[3/3] PG JSONB — CREATE B-Tree INDEX...' });
  const { time: tBtreeCreate } = await timer(() => pg.query(`CREATE INDEX idx_bench_jsonb_btree ON bench_jsonb ((data->>'category'))`));
  R.createIndex = tBtreeCreate;

  emit({ step: 'jsonb_btree_q', message: '[3/3] PG JSONB — SELECT filter (B-Tree)...' });
  const { time: tBtreeSelect, result: rBtreeSelect } = await timer(() => pg.query(`SELECT * FROM bench_jsonb WHERE data->>'category' = $1`, [filterCat]));
  R.selectIndexed = tBtreeSelect;

  emit({ step: 'jsonb_storage', message: '[3/3] PG JSONB — Measuring storage (B-Tree)...' });
  const szBtree = await pg.query(`SELECT pg_table_size('bench_jsonb') AS data, pg_indexes_size('bench_jsonb') AS idx, pg_total_relation_size('bench_jsonb') AS total`);
  R.storage = { data: parseInt(szBtree.rows[0].data), index: parseInt(szBtree.rows[0].idx), total: parseInt(szBtree.rows[0].total) };

  await pg.query('DROP INDEX idx_bench_jsonb_btree');

  emit({ step: 'jsonb_gin', message: '[3/3] PG JSONB — CREATE GIN INDEX (bonus)...' });
  const { time: tGinCreate } = await timer(() => pg.query('CREATE INDEX idx_bench_jsonb_gin ON bench_jsonb USING GIN(data)'));
  R.ginCreateIndex = tGinCreate;

  emit({ step: 'jsonb_gin_q', message: '[3/3] PG JSONB — SELECT filter (GIN)...' });
  const { time: tGinSelect } = await timer(() => pg.query('SELECT * FROM bench_jsonb WHERE data @> $1::jsonb', [JSON.stringify({ category: filterCat })]));
  R.ginSelectIndexed = tGinSelect;

  emit({ step: 'jsonb_gin_storage', message: '[3/3] PG JSONB — Measuring storage (GIN)...' });
  const szGin = await pg.query(`SELECT pg_table_size('bench_jsonb') AS data, pg_indexes_size('bench_jsonb') AS idx, pg_total_relation_size('bench_jsonb') AS total`);
  R.ginStorage = { data: parseInt(szGin.rows[0].data), index: parseInt(szGin.rows[0].idx), total: parseInt(szGin.rows[0].total) };

  emit({ step: 'jsonb_update', message: '[3/3] PG JSONB — UPDATE 1 row...' });
  const { time: tUpd } = await timer(() => pg.query(`UPDATE bench_jsonb SET data = data || $1::jsonb WHERE id = 1`, [JSON.stringify({ comment: 'updated_comment' })]));
  R.update = tUpd;
